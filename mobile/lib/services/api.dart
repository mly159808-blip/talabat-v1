...
class Api {
  ...
  static Future<dynamic> getAuthed(String path, String? token) async {
    if (token == null || token.isEmpty) throw Exception('يجب تسجيل الدخول');
    final r = await http.get(Uri.parse(base + path), headers: {'Authorization': 'Bearer $token'});
    if (r.statusCode >= 400) {
  ...
  static Future<dynamic> post(String path, Map body, {String? token}) async {
    final r = await http.post(Uri.parse(base + path), headers: {'Content-Type': 'application/json', if (token != null) 'Authorization': 'Bearer $token'}, body: jsonEncode(body));
    if (r.statusCode >= 400) {
      final data = jsonDecode(r.body);
      throw Exception(data['message'] ?? 'خطأ');
    }
